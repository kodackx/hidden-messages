...
class TestLLMErrorHandling:
    """Test how the system handles LLM API errors"""
    ...
    async def test_invalid_api_key_handling(self):
        ...
        try:
            manager = HiddenMessageAgent()
            agents_config = {
                "test": {"provider": "openai", "role": "bystander"}
            }
            ...
            assert response is None, "Response should be None on error"
            assert error is not None, "Error message should be provided"
            print(f"✅ Error handled gracefully: {error[:100]}...")
    ...
    @pytest.mark.asyncio
    async def test_no_api_key_behavior(self):
        ...
        try:
            ...
            print(f"✅ Missing key handled: {error[:100]}...")
